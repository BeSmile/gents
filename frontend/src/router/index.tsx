 ...
 * @LastEditors: BeSmile
 * @LastEditTime: 2021-01-19 11:29:29
 */
...
const ProtectRouter = ({ path, component: Component, ...props }) => {
  const BaseRouter = (props) => {
    ...
    return !loggedIn ? <Redirect to="/login" /> : <Component />;
  };

  const BaseComponent = connect(mapToProps)(BaseRouter);

  return <Route path={path} {...props} component={BaseComponent} />;
};

//生成子路由页面
const generateChildRouter = (
  path: string,
  routes: Array<any>
  ...
  let RouteUI = routes.reduce((UI, item) => {
    ...
    const RenderRoute =
      isArrayStatus && item.authority.length > 0 ? ProtectRouter : Route;
    UI.push(
      <RenderRoute
        exact
        key={`${path}${item.path}`}
...
const generateRouter = (routes: Array<any>): Array<React.ReactElement> => {
  const RoutersUI = routes.reduce((RouteUI, item) => {
    const LayoutUI = isEmpty(item.component)
      ? React.Fragment
      : lazy(() => import(`../${item.component}`));
    RouteUI.push(
      <Route key={item.path} path={item.path}>
        <LayoutUI>
          <Switch>{generateChildRouter(item.path, item.routes)}</Switch>
        </LayoutUI>
      </Route>
    );
    return RouteUI;
      ...
      <ThemeLayout>
        ...
        <Suspense fallback={<div>Loading...</div>}>
          <Switch>
            ...
            <ProtectRouter path="/domain" component={() => <div>domain</div>} />
          </Switch>
        </Suspense>
      </ThemeLayout>
    </Router>
